...
const conf = new String(PATH.basename(__filename, PATH.extname(__filename)));
const base = String(conf);
// the cur environment
conf.env = !process.env.NODE_ENV? 'development' : process.env.NODE_ENV.toLowerCase();
// the computed filesystem properties
conf.fs       = {};
conf.fs.root  = __dirname;
conf.fs.ext   = PATH.extname(__filename);
conf.fs[base] = PATH.join(conf.fs.root, base);

// exports a stream containing the resulting config following these rules:
module.exports = toStream(FS.stat, conf.fs[base])
    ...
    .map(path => {
        ...
        if (typeof mod != 'function')
            throw new TypeError(`Invalid module '${path}', expecting function, got: ${typeof mod}`);
